...
class Manager:
    ...
    # def team_info_from_data(self, gw_data):
    #     """Creates a team from current gameweek data

    #     Creates a team which is to be associated with the manager for
    #     the current gameweek.

    #     Arguments:
    #         gw_data {[dict]} -- A list of dicts, containing information about
    #                             the players in the manager's team.

    #     Returns:
    #         Team -- A team object to be associated with the manager.
    #     """
    #     return Team(gw_data['picks'])

    def fetch_gw_team(self, gw):
        info = session.get(TEAM_GW_INFO_URL.format(id=self.team_id,
                                                   gw=gw)).json()
...
def get_current_state():
    ...
    current_info = api.DynamicInfo.gameweek_stats()
